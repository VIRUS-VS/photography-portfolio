...
const registerAdmin = async (req, res) => {
  ...
  try {
    ...
    if (admin) res.status(201).json({ _id: admin._id, email: admin.email, token: generateToken(admin._id) });
...
const updateAdminProfile = async (req, res) => {
  try {
    ...
    if (admin) {
      ...
      res.json({ _id: updatedAdmin._id, email: updatedAdmin.email, token: generateToken(updatedAdmin._id) });
    } else {
