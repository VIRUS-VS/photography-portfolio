...
const ManageGalleryPage = () => {
  ...
  const fetchGalleryAndPhotos = async () => {
    try {
      setLoading(true);
      const galleryRes = await axios.get(`http://localhost:5000/api/galleries/${id}`);
      const photosRes = await axios.get(`http://localhost:5000/api/photos/${id}`);
  ...
  const uploadFilesHandler = async (e) => {
    ...
    try {
      ...
      await axios.post(`http://localhost:5000/api/photos/${id}`, formData, config);
  ...
  const deletePhotoHandler = async (photoId) => {
    if (window.confirm('Are you sure?')) {
      try {
        ...
        await axios.delete(`http://localhost:5000/api/photos/${photoId}`, config);
        fetchGalleryAndPhotos();
      } catch (error) {
   ...
   const deleteGalleryHandler = async () => {
      if (window.confirm('Are you sure you want to PERMANENTLY DELETE this entire gallery and all its contents? This cannot be undone.')) {
        try {
            const config = { headers: { Authorization: `Bearer ${adminInfo.token}` } };
